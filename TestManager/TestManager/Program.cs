...
namespace TestManager
{
    public class Program
    {
        ...
        try
        {
            ...
            pipeline.Commands.AddScript(testPath+"Get_PXE.ps1");
        ...
        static void FTP_Upload(string TR_ID)
        {
            Runspace runspace = RunspaceFactory.CreateRunspace();
            runspace.Open();
            Pipeline pipeline = runspace.CreatePipeline();
            ...
            try
            {
                process_log("FTP_Upload +++ "+testPath+"Upload.ps1 "+TR_ID);
                pipeline.Commands.AddScript(testPath+"Upload.ps1 "+TR_ID);
                var result = pipeline.Invoke();
            }    
            catch (Exception ex)
            {
        ...
        static bool Update_Job_Status(string TR_ID, string TR_Test_Result)
        {
            if(TR_Test_Result == "True")
            {
                TR_Test_Result = "Pass";
            }
            else
            ...
            {
                // pipeline.Commands.AddScript(testPath+"RunAs.ps1");
                pipeline.Commands.AddScript(testPath+"Update_Job_Status.ps1 "+TR_ID+" "+TR_Test_Result);
                var result = pipeline.Invoke();
                ...
                foreach (string job in Job_List)
                {
                    ...
                    // {
                        ...
                        try
                        {
                            if( (job_index % 2) == 1)
                            {
                                process_log("  Item["+(job_index % 2)+"] -> 執行 DLL");
                                result = Execute_dll(downloadPath+job);
                            }
                            else
                            {
                                // step 4. upload 測試 log files to FTP
                                process_log(" upload log to FTP 子目錄: "+job);
                                if(job != "0")
                                {
                                    FTP_Upload(job);
                                    Update_Job_Status(job, result.ToString());   //Update test result
                                }
                                else
                                {
                                    Update_Job_Status("common_bios_pxeboot_default.dll", "Pass");   //Update test result
                                }    
                            }
